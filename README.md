...
* 关于闪屏的bug: 这个bug 出现的原因 可能是因为 定时器 的间隔  和  屏幕切换之前时间 有点接近， 所以将 初始的定时器 修改为 7s, 能有效的规避这个bug, 但是bug 并没有从根本上得以解决, 个人觉得是因为 页面动画的性能没有我想象的那么好, 有一个博客是说, 如果同一个页面中有超过5 个 元素在运动, 采用 canvas 绘制的性能要好于 css3 的方案

* 好吧，在跨浏览器方面，以后还是应该在别的浏览器里跑一下，这个网页的性能感觉不太好，第一次觉得css3的动画还是不能滥用，`tween算法`  +  `canvas` 的实现方案还是更好一些的。
